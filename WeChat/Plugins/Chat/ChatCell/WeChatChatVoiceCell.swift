...
//聊天录音tableViewCell
class WeChatChatVoiceCell:WeChatChatBaseCell {
    ...
    override func setMessage(message: ChatMessage) {
        ...
        let indexOfBackConstraint = constraints.indexOfObjectPassingTest { (constraint, idx, stop) in
            return (constraint.firstItem as! UIView).tag == self.voiceIndicatorImageTag && (constraint.firstAttribute == NSLayoutAttribute.Left || constraint.firstAttribute == NSLayoutAttribute.Right)
        }
